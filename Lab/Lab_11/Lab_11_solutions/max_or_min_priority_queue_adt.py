...
'''
A priority queue abstract data type with priorities given to either maximal or minimal values
'''
...
class PriorityQueue():
    ...
    def __init__(self, capacity = min_capacity, key = lambda x: x):
    ...
    def _bubble_up(self, i):
        if i > 1 and self.key(self._data[i]) > self.key(self._data[i // 2]):
            self._data[i // 2], self._data[i] = self._data[i], self._data[i // 2]
            self._bubble_up(i // 2)

    def _bubble_down(self, i):
        child = 2 * i
        if child < self._length and self.key(self._data[child + 1]) > self.key(self._data[child]):
            child += 1
        if child <= self._length and self.key(self._data[i]) < self.key(self._data[child]):
...
class MaxPriorityQueue(PriorityQueue):
    def __init__(self, capacity = PriorityQueue.min_capacity):
        '''
        >>> pq = MaxPriorityQueue(capacity = 4)
        >>> L = [13, 13, 4, 15, 9, 4, 5, 14, 4, 11, 15, 2, 17, 8, 14, 12, 9, 5, 6, 16]
...
class MinPriorityQueue(PriorityQueue):
    def __init__(self, capacity = PriorityQueue.min_capacity):
        ...
        [None, 4, 13, 13]    10
        [None, 4, 13, 13, 15]    10
        [None, 4, 9, 13, 15, 13]    10
        [None, 4, 9, 4, 15, 13, 13]    10
        [None, 4, 9, 4, 15, 13, 13, 5]    10
        [None, 4, 9, 4, 14, 13, 13, 5, 15]    10
        [None, 4, 4, 4, 9, 13, 13, 5, 15, 14]    10
        [None, 4, 4, 4, 9, 11, 13, 5, 15, 14, 13]    20
        [None, 4, 4, 4, 9, 11, 13, 5, 15, 14, 13, 15]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 15, 14, 13, 15, 13]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 15, 14, 13, 15, 13, 17]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 15, 14, 13, 15, 13, 17, 8]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 15, 14, 13, 15, 13, 17, 8, 14]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 12, 14, 13, 15, 13, 17, 8, 14, 15]    20
        [None, 2, 4, 4, 9, 11, 4, 5, 9, 14, 13, 15, 13, 17, 8, 14, 15, 12]    20
        [None, 2, 4, 4, 5, 11, 4, 5, 9, 9, 13, 15, 13, 17, 8, 14, 15, 12, 14]    20
        [None, 2, 4, 4, 5, 11, 4, 5, 9, 6, 13, 15, 13, 17, 8, 14, 15, 12, 14, 9]    20
        [None, 2, 4, 4, 5, 11, 4, 5, 9, 6, 13, 15, 13, 17, 8, 14, 15, 12, 14, 9, 16]    40
        >>> for _ in range(len(L)):
        ...     print(f'{pq.delete():2d} {pq._data[: len(pq) + 1]}    {len(pq._data)}')
         2 [None, 4, 5, 4, 6, 11, 4, 5, 9, 9, 13, 15, 13, 17, 8, 14, 15, 12, 14, 16]    40
         4 [None, 4, 5, 4, 6, 11, 13, 5, 9, 9, 13, 15, 16, 17, 8, 14, 15, 12, 14]    40
         4 [None, 4, 5, 5, 6, 11, 13, 8, 9, 9, 13, 15, 16, 17, 14, 14, 15, 12]    40
         4 [None, 5, 6, 5, 9, 11, 13, 8, 12, 9, 13, 15, 16, 17, 14, 14, 15]    40
         5 [None, 5, 6, 8, 9, 11, 13, 14, 12, 9, 13, 15, 16, 17, 15, 14]    40
         5 [None, 6, 9, 8, 9, 11, 13, 14, 12, 14, 13, 15, 16, 17, 15]    40
         6 [None, 8, 9, 13, 9, 11, 15, 14, 12, 14, 13, 15, 16, 17]    40
         8 [None, 9, 9, 13, 12, 11, 15, 14, 17, 14, 13, 15, 16]    40
         9 [None, 9, 11, 13, 12, 13, 15, 14, 17, 14, 16, 15]    40
         9 [None, 11, 12, 13, 14, 13, 15, 14, 17, 15, 16]    20
        11 [None, 12, 13, 13, 14, 16, 15, 14, 17, 15]    20
        12 [None, 13, 14, 13, 15, 16, 15, 14, 17]    20
        13 [None, 13, 14, 14, 15, 16, 15, 17]    20
        13 [None, 14, 15, 14, 17, 16, 15]    20
        14 [None, 14, 15, 15, 17, 16]    10
        14 [None, 15, 16, 15, 17]    10
        15 [None, 15, 16, 17]    10
        15 [None, 16, 17]    10
        16 [None, 17]    10
        17 [None]    10
        '''
        super().__init__(capacity = capacity, key = lambda x: -x)
