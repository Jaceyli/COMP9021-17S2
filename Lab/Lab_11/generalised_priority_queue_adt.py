...
class PriorityQueue():
    ...
    def insert(self, element):
        ...
        if data in self._exist:
            if self._data[self._exist[data]][1] != priority:
                self._change_priority(element, self._exist[data])
                return
        else:
            self._length += 1
            self._data[self._length] = [data, priority]
            self._exist[data] = self._length
    ...
    def _change_priority(self, element, i):
        if element[1] > self._data[i][1]:
            self._data[i][1] = element[1]
            self._bubble_up(i)
        else:
            self._data[i][1] = element[1]
            self._bubble_down(i)
    ...
    def _bubble_up(self, i):
        if i > 1 and self._data[i][1] > self._data[i // 2][1]:
            ...
            self._exist[self._data[i][0]] = i
            self._exist[self._data[i // 2][0]] = i // 2
            self._bubble_up(i // 2)

    def _bubble_down(self, i):
        child = 2 * i
        # print(child, self._length, self._data[child][1])
        # print(self._data[child + 1][1])
        ...
        if child <= self._length and self._data[i][1] < self._data[child][1]:
            ...
            self._exist[self._data[i][0]] = i
            self._exist[self._data[child][0]] = child
